...
// TestDispatchMessageIncludesQuotedImage verifies that when a user replies to
// a Feishu image (or rich-text post containing an image), cc-connect fetches
// the quoted image and threads it through to core.Message.Images so the agent
// receives the actual image rather than a "[image]" textual placeholder.
// This is an explicit reply OUTSIDE a thread (thread_id empty), so the quote
// gate fires and quoted attachments are pulled in. (Cherry-picked from
// upstream PR #944, dispatchMessage signature adapted to local 12-arg shape.)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			p.dispatchMessage(
				...
				parentMessageID,
				"",    // threadID — explicit reply outside a thread
				false, // isThread
			)
