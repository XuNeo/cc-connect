...
func TestQueuedMessagePreservesFiles(t *testing.T) {
	...
	// Mid-stream inject sends the queued message straight to the agent's stdin
	// (no "process after" ack — MsgMessageQueued was dropped). Wait for the
	// agent to record both sends before releasing the blocked turn.
